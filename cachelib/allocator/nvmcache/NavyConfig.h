...
class NavyConfig {
 ...
 public:
  ...
  bool usesZonedFile() const noexcept { return zonedDevice_; }
  ...
  void setRaidFiles(std::vector<std::string> raidPaths,
                    uint64_t fileSize,
                    bool truncateFile = false);
  // Set zoned device enabled
  void setZonedDevice(bool zonedDevice) {
    zonedDevice_  = zonedDevice;
  }
